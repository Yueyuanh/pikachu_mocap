...
def fix_alpha() -> None:
    for mat in bpy.data.materials:
        if not hasattr(mat, "node_tree") or mat.node_tree is None or mat.node_tree.nodes is None:
            continue
        # Blender 5.x 不保证含“Principled BSDF”节点，遍历查找同名类型节点更稳妥
        ...
        if hasattr(visual, "material") and hasattr(visual.material, "name"):
            ...
            if material is None:
                ...
                if hasattr(visual.material, "color") and visual.material.color and visual.material.color.rgba:
                    ...
                    # Blender 5.x 新建材质不保证默认“Principled BSDF”节点存在，取不到就手动创建
                    ...
                    # 确保连到 Material Output，否则颜色不生效
                    if not principled_node.outputs["BSDF"].links:
